...
def record_video():
    command = [ "ffmpeg", "-f" , "v4l2", "-framerate", "60", "-video_size", "1280x720", "-i", "/dev/video0", "-c:v", "libx264", "-preset", "ultrafast", "outputsc2.mp4" ]
    taskset_command = ["taskset", "-c", "0-3"] + command
    subprocess.run(taskset_command)

#process = multiprocessing.Process(target=record_video)
#process.start()
